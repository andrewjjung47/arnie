...
def mfe(
    ...
    **kwargs,
    ...
    if linear and pkg not in ["vienna", "contrafold", "eternafold"]:
        print(
            "Warning: LinearFold only implemented for vienna, contrafold, eternafold."
        )
    ...
    if pkg == "vienna":
        if linear:
            if return_dG_MFE:
                struct, dG_MFE = mfe_linearfold_(
                    seq,
                    package="vienna",
                    return_dG_MFE=return_dG_MFE,
                    beam_size=beam_size,
                )
            else:
                struct = mfe_linearfold_(
                    seq, package="vienna", return_dG_MFE=return_dG_MFE
                )
        else:
            struct = mfe_vienna_(
                seq,
        ...
        if linear:
            if return_dG_MFE:
                ...
                struct = mfe_linearfold_(
                    seq, package="contrafold", return_dG_MFE=return_dG_MFE
                )
        else:
            struct = mfe_contrafold_(
                seq,
                version=version,
                T=T,
                constraint=constraint,
        ...
        if linear:
            if return_dG_MFE:
                ...
                struct = mfe_linearfold_(
                    seq, package="eternafold", return_dG_MFE=return_dG_MFE
                )

        else:

            if (
                "eternafoldparams" in package_locs.keys()
                and "eternafold" not in package_locs.keys()
            ):
                struct = mfe_contrafold_(
                    seq,
                    version=version,
                    T=T,
                    constraint=constraint,
                    param_file=package_locs["eternafoldparams"],
                    viterbi=viterbi,
                )
                ...
                efold_param_file = (
                    os.environ["ETERNAFOLD_PARAMETERS"]
                    if os.environ.get("ETERNAFOLD_PARAMETERS")
                    else package_locs["eternafold"]
                    + "/../parameters/EternaFoldParams.v1"
                )
                if not os.path.exists(efold_param_file):
                    raise RuntimeError(
                        "Error: Parameters not found at %s" % efold_param_file
                    )
                else:
                    struct = mfe_contrafold_(
                        seq,
                        ...
                        T=T,
                        constraint=constraint,
                        DIRLOC=package_locs["eternafold"],
                        param_file=efold_param_file,
                        viterbi=viterbi,
                        probing_signal=probing_signal,
                        probing_kws=probing_kws,
                    )
...
def mfe_vienna_(
    ...
    **kwargs,
    ...
    command = [
        ...
        str(T),
        "-p0",
    ]  # p0 doesn't predict bpps, saves time
...
def mfe_rnastructure_(
    ...
    if param_file is not None:
        raise ValueError(
            "Cannot run RNAstructure with non-default RNA parameters as specified in: %s"
            % param_file
        )
    if version is not None:
        raise ValueError(
            "Cannot run RNAstructure with non-default version: %s" % version
        )
    if (shape_signal is not None) and (shape_file is not None):
        raise ValueError(
            "Please specify SHAPE reactivities either as a list or in a SHAPE reactivity file"
        )
    if (dms_signal is not None) and (dms_file is not None):
        raise ValueError(
            "Please specify DMS reactivities either as a list or in a DMS reactivity file"
        )
    ...
    if not pseudo:
        ...
        if constraint is not None:
            raise ValueError(
                "Cannot run RNAstructure with constraints and pseudoknots."
            )
    ...
    if dms_signal is not None:
        if len(dms_signal) != len(seq):
            raise RuntimeError(
                "DMS signal used with RNAstructure must have same length as the sequence."
            )
    ...
    if shape_signal is not None:
        if len(shape_signal) != len(seq):
            raise RuntimeError(
                "SHAPE signal used with RNAstructure must have same length as the sequence."
            )
...
def mfe_contrafold_(
    ...
    if probing_signal is not None:
        command = command + [
            "--evidence",
            "--params",
            package_locs["eternafold"]
            + "/../parameters/EternaFoldParams_PLUS_POTENTIALS.v1",
            "--numdatasources",
            "1",
        ]
    ...
    if constraint is not None:
        ...
        if probing_signal is None:
            convert_dbn_to_contrafold_input(
                seq, "".join(["." for x in range(len(seq))]), fname
            )
...
def mfe_linearfold_(
    seq, bpps=False, package="contrafold", beam_size=100, return_dG_MFE=False
):
    ...
    if package == "contrafold":
        ...
        raise ValueError("Invalid package %s for linearfold" % package)
